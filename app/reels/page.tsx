...
const fallbackReels: ReelDoc[] = [
  { documentId: "fuse-reel-fayrouz-1", title: "كاهي فيروز طالع حار", caption: "كاهي وقيمر وشاي عراقي... فطور يفتح النفس.", restaurant: "فيروز", restaurantSlug: "fayrouz", category: "فطور", offer: "خصم 20%", menuItem: "كاهي وقيمر", price: 4500, deliveryTime: "20-30 دقيقة", image: "/images/m2.jpg", likes: 128, views: 2400 },
  { documentId: "fuse-reel-shalteta-1", title: "مشلتت جبن يسحب", caption: "طازج، حار، وجبن يذوب من أول لقمة.", restaurant: "شلتتة", restaurantSlug: "shalteta", category: "فطور", offer: "توصيل سريع", menuItem: "مشلتت جبن", price: 7500, deliveryTime: "25-35 دقيقة", image: "/images/m3.jpg", likes: 96, views: 1800 },
  { documentId: "fuse-reel-khan-1", title: "وجبة عراقية من خان قدوري", caption: "رز ومرگ ودجاج مشوي... غداء عراقي كامل.", restaurant: "خان قدوري", restaurantSlug: "khan", category: "مشاوي", offer: "وجبات يومية", menuItem: "دجاج مشوي", price: 9000, deliveryTime: "30-40 دقيقة", image: "/images/khan.jpg", likes: 143, views: 3100 },
  { documentId: "fuse-reel-alforn-1", title: "مناقيش الفرن", caption: "مناقيش ومعجنات حارة بطعم يوصلك للفرن مباشرة.", restaurant: "الفرن", restaurantSlug: "alforn", category: "بيتزا", offer: "جديد", menuItem: "مناقيش جبن", price: 6000, deliveryTime: "25-35 دقيقة", image: "/images/m5.jpg", likes: 71, views: 950 },
];
...
export default function ReelsPage() {
  ...
  function goBack() { if (window.history.length > 1) window.history.back(); else window.location.assign("/"); }
  ...
  return (
    <main dir="rtl" className="page">
          ...
          return (
            <article className="reel" data-reel-id={id} key={id}>
              ...
              <header className="topbar">
                <button type="button" className="glass" aria-label="رجوع" onClick={goBack}><Icon name="back" /></button>
              ...
              <section className="content">
                ...
                {!isCustomerReel ? <><div className="product-card"><div><small>اطلب من داخل الريل</small><b>{reel.menuItem || reel.title || "وجبة FUSE"}</b><strong>{formatIQD(reel.price)}</strong></div><button onClick={() => addToCart(reel)} aria-label="إضافة للسلة">+</button></div><div className="cta-row"><Link href={`/restaurants/${slug}`} className="order-btn"><Icon name="cart" /> اطلب الآن</Link><button onClick={() => addToCart(reel)} className="quick-add">إضافة سريعة</button></div></> : null}
              </section>
            </article>
          );
        })}
      ...
      <nav className="bottom-nav" aria-label="التنقل الرئيسي">
        ...
        <Link href="/reels" className="active"><b>▶</b><span>ريلز</span></Link>
      ...
      <style jsx>{`
        ...
        .feed{height:calc(100% - 78px);overflow-y:auto;scroll-snap-type:y mandatory;overscroll-behavior-y:contain;scrollbar-width:none}.feed::-webkit-scrollbar{display:none}
        ...
        .topbar{position:absolute;top:0;left:0;right:0;z-index:5;padding:calc(max(10px,env(safe-area-inset-top)) + 2px) 14px 8px;display:grid;grid-template-columns:46px 1fr auto;align-items:center}.top-actions{display:flex;gap:8px}.glass{width:44px;height:44px;border:1px solid rgba(255,255,255,.18);border-radius:50%;display:grid;place-items:center;background:rgba(10,10,10,.38);backdrop-filter:blur(14px);color:#fff;text-decoration:none}.create{font-size:29px;line-height:1}.tabs{justify-self:center;display:flex;gap:20px;font-size:15px;text-shadow:0 2px 8px #000}.tabs b{position:relative}.tabs b:after{content:"";position:absolute;width:20px;height:3px;border-radius:9px;background:#ff6700;bottom:-8px;left:50%;transform:translateX(-50%)}.tabs span{color:rgba(255,255,255,.68)}
        .actions{position:absolute;z-index:6;left:12px;bottom:178px;display:grid;gap:14px;justify-items:center}.actions button{border:0;background:none;color:#fff;padding:0;display:grid;justify-items:center;gap:4px}.actions button span{width:46px;height:46px;border-radius:50%;display:grid;place-items:center;background:rgba(15,15,15,.45);border:1px solid rgba(255,255,255,.12)}.actions button b{font-size:10px}.actions button.active span{color:#ff5a36;background:#fff}.avatar{position:relative;width:50px;height:50px;border-radius:50%;padding:2px;border:2px solid #fff;display:block}.avatar img{width:100%;height:100%;object-fit:cover;border-radius:50%}.avatar i{position:absolute;right:50%;bottom:-9px;transform:translateX(50%);width:20px;height:20px;border-radius:50%;display:grid;place-items:center;background:#ff5a00;color:#fff;font-style:normal}
        .content{position:absolute;z-index:5;right:14px;left:70px;bottom:16px;text-shadow:0 2px 10px rgba(0,0,0,.85)}.restaurant-line{display:flex;align-items:center;gap:7px;margin-bottom:7px}.restaurant-line b{font-size:15px}.restaurant-line em{width:17px;height:17px;display:grid;place-items:center;border-radius:50%;background:#ff6200;font-size:11px;font-style:normal}.restaurant-line strong{font-size:10px;padding:5px 8px;border-radius:999px;background:#ff5a00}h1{margin:0 0 6px;font-size:22px}p{margin:0 0 9px;font-size:13px;line-height:1.6;color:rgba(255,255,255,.88);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.stats{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px}.stats span{font-size:9px;padding:5px 8px;border-radius:999px;background:rgba(0,0,0,.45);border:1px solid rgba(255,255,255,.12)}
        .product-card{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:11px 12px;border-radius:18px;background:rgba(12,12,12,.62);border:1px solid rgba(255,255,255,.16);margin-bottom:9px;text-shadow:none}.product-card div{min-width:0;display:grid;gap:2px}.product-card small{font-size:9px;color:#ff9a55}.product-card b{font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.product-card strong{font-size:13px;color:#ff7a00}.product-card button{width:42px;height:42px;border:0;border-radius:50%;background:linear-gradient(135deg,#ff8a00,#ff3d00);color:#fff;font-size:25px}.cta-row{display:grid;grid-template-columns:1.25fr .75fr;gap:8px}.order-btn,.quick-add{height:46px;border-radius:15px;border:0;display:flex;align-items:center;justify-content:center;gap:7px;font-weight:900;font-family:inherit}.order-btn{background:linear-gradient(135deg,#ff7a00,#ff3d00);color:#fff;text-decoration:none}.quick-add{background:#fff;color:#101010}
        .bottom-nav{position:fixed;z-index:40;left:50%;bottom:0;transform:translateX(-50%);width:100%;max-width:430px;height:78px;padding:6px 8px calc(6px + env(safe-area-inset-bottom));display:grid;grid-template-columns:repeat(5,1fr);background:rgba(255,250,244,.98);border-top:1px solid rgba(0,0,0,.08);box-shadow:0 -8px 28px rgba(0,0,0,.22)}.bottom-nav a{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;text-decoration:none;color:#777;font-size:10px;font-weight:900;border-radius:16px}.bottom-nav a b{font-size:21px;line-height:1}.bottom-nav a.active{color:#ff5a00;background:#fff0e6}
        .toast{position:fixed;z-index:50;left:50%;bottom:96px;transform:translateX(-50%);background:#fff;color:#111;padding:12px 18px;border-radius:999px;font-weight:900;white-space:nowrap}
        @media(min-width:700px){.feed,.bottom-nav{width:min(430px,100%);margin:auto}.feed{box-shadow:0 0 80px rgba(0,0,0,.7)}}@media(max-width:360px){.content{right:12px;left:67px}.actions{left:8px}.actions button span{width:42px;height:42px}h1{font-size:19px}}
      `}</style>
    </main>
  );
}
