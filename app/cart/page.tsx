...
function isFallbackItem(id: string) {
  return /^(fayrouz|shalteta|khan|forn)-\d+$/.test(id);
}
...
async function validateCart(items: FuseCartItem[]) {
  ...
  const restaurant = String(items[0].restaurant || "").trim();

  if (items.some((item) => String(item.restaurantId || item.restaurant).trim() !== String(restaurantId || restaurant).trim())) {
  ...
  if (restaurantId) {
    ...
    if (restaurantSnap.exists()) {
      ...
      const open = data.active !== false && data.open !== false && data.isOpen !== false && data.status !== "مغلق";
      if (!open) throw new Error("المطعم مغلق حالياً ولا يستقبل طلبات.");
    }
  }
  ...
  for (const item of items) {
    ...
    if (!menuSnap.exists()) {
      if (!isFallbackItem(item.id)) throw new Error(`الصنف ${item.name} لم يعد متوفراً.`);
      verified.push({ ...item, price: Math.max(0, Number(item.price || 0)), qty: Math.max(1, Math.min(50, item.qty)) });
      continue;
    }
    ...
    const itemRestaurant = String(data.restaurant || data.restaurantName || "").trim();
    if ((restaurantId && itemRestaurantId && itemRestaurantId !== restaurantId) || (!restaurantId && itemRestaurant && itemRestaurant !== restaurant)) {
    ...
    verified.push({
      ...
      price,
      qty: Math.max(1, Math.min(50, Math.round(item.qty))),
    });
  }
...
export default function CartPage() {
  ...
  function changeQty(item: FuseCartItem, nextQty: number) {
    setItems(updateFuseCartQty(item.id, Math.min(50, nextQty)));
  }
  ...
  async function submitOrder() {
    ...
    try {
      ...
      const restaurantName = verifiedItems[0]?.restaurant || restaurant;
  ...
  return (
    <main dir="rtl" className="app">
      <header className="top"><button className="back" type="button" onClick={() => history.length > 1 ? history.back() : router.push("/restaurants")}>‹</button><div className="title"><h1>السلة</h1><p>{items.length ? `${items.reduce((sum, item) => sum + item.qty, 0)} قطعة من ${restaurant}` : "ابدأ بإضافة وجبات"}</p></div><Link className="support" href="/support">دعم</Link></header>
      {!items.length ? <section className="empty"><div>🛒</div><h2>السلة فارغة</h2><p>اختار مطعماً وأضف وجبتك.</p><Link href="/restaurants">تصفح المطاعم</Link></section> : <>
        <section className="group"><div className="groupHead"><h2>{restaurant}</h2><small>{items.length} صنف</small></div>{items.map((item) => <article className="item" key={`${item.restaurantId || restaurant}-${item.id}`}><div className="thumb">{item.name.slice(0, 1)}</div><div className="info"><h3>{item.name}</h3><p>{item.category || "عام"} · {formatIQD(item.price)}</p><div className="row"><div className="qty"><button type="button" onClick={() => changeQty(item, item.qty - 1)}>−</button><b>{item.qty}</b><button type="button" onClick={() => changeQty(item, item.qty + 1)}>+</button></div><strong>{formatIQD(item.price * item.qty)}</strong></div></div></article>)}</section>
        <section className="summary"><h2>ملخص الطلب</h2><div><span>المجموع الفرعي</span><b>{formatIQD(totals.subtotal)}</b></div><div><span>التوصيل</span><b>{formatIQD(totals.deliveryFee)}</b></div><div className="total"><span>الإجمالي</span><b>{formatIQD(totals.total)}</b></div><small>يُراجع السعر والتوفر مرة ثانية عند تأكيد الطلب.</small></section>
        ...
        <button className="checkout" type="button" onClick={submitOrder} disabled={saving}>{saving ? "جاري فحص الأسعار وتثبيت الطلب..." : `تأكيد الطلب · ${formatIQD(totals.total)}`}</button><button className="clear" type="button" onClick={clearCart} disabled={saving}>تفريغ السلة</button>
      </>}
      {message && <div className="message ok">{message}</div>}{error && <div className="message bad">{error}</div>}
