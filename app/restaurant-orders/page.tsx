...
const PAGE_VERSION = "FUSE_RESTAURANT_ORDERS_V7_ENV_LIVE";
...
function text(order: LiveOrder, keys: string[], fallback: string): string {
...
function num(order: LiveOrder, keys: string[]): number | null {
  for (const key of keys) {
    const value = order[key];
...
function items(order: LiveOrder): any[] {
  const raw =
    order.items ||
    order.cart ||
    order.orderItems ||
...
function status(order: LiveOrder): string {
  return text(order, ["status", "orderStatus", "state"], "جديد");
}
...
export default function RestaurantOrdersPage() {
  ...
  useEffect(() => {
    ...
    if (missing.length > 0) {
      setError("Firebase ENV ناقصة: " + missing.join(", "));
      setLoading(false);
      return;
    }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      newOrders: orders.filter((order) => {
        const value = String(status(order)).toLowerCase();
        return value === "جديد" || value.includes("new") || value.includes("pending");
      }).length,
    };
  }, [orders]);
                  ...
                  <div className="mt-5 grid gap-3 md:grid-cols-3">
                    <div className="rounded-2xl bg-black/25 p-4">
                      ...
                      <p className="mt-1 font-bold">
                        {text(order, ["customerName", "name", "clientName", "userName"], "زبون غير محدد")}
                      </p>
                    </div>
                    ...
                    <div className="rounded-2xl bg-black/25 p-4">
                      ...
                      <p className="mt-1 font-bold">
                        {text(order, ["customerPhone", "phone", "mobile", "clientPhone"], "غير محدد")}
                      </p>
                    </div>
                    ...
                    <div className="rounded-2xl bg-black/25 p-4">
                      ...
                      <p className="mt-1 font-bold">
                        {text(order, ["address", "customerAddress", "deliveryAddress", "locationText"], "غير محدد")}
                      </p>
                    </div>
                  </div>
