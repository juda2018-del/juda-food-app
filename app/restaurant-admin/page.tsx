...
export default function RestaurantAdminPage() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(query(collection(db, "restaurants")), (snapshot) => {
      ...
      const own = all.find(
        (item) => item.documentId === ownKey || getRestaurantName(item).toLowerCase() === ownKey.toLowerCase()
      );
      const scoped = own ? [own] : [];
  ...
  useEffect(() => {
    ...
    const menuQuery = query(collection(db, "menu"), where("restaurantId", "==", selectedId));
    const ordersQuery = query(collection(db, "orders"), where("restaurantId", "==", selectedId));
  ...
  const sortedOrders = useMemo(
    () => [...orders].sort((a, b) => String(b.orderId || b.documentId).localeCompare(String(a.orderId || a.documentId))),
    [orders]
  );
  ...
  async function toggleRestaurant() {
    ...
    try {
      ...
      flash(next ? "تم تشغيل المطعم." : "تم إطفاء المطعم وإخفاؤه من التطبيق.");
    } catch (e) {
    ...
    <main dir="rtl" className="page">
      <section className="shell">
        <header className="topbar">
          <div><small>FUSE Restaurant</small><h1>{role === "admin" ? "إدارة المطاعم والمنيو" : `لوحة ${selectedName || "المطعم"}`}</h1></div>
        ...
        {role === "admin" ? (
          <section className="panel">
            <div className="panel-head"><div><small>Restaurant Setup</small><h2>{editingRestaurantId ? "تعديل المطعم" : "إضافة مطعم جديد"}</h2></div></div>
            <div className="form-grid">
              ...
              <label className="switch wide"><input type="checkbox" checked={restaurantForm.open} onChange={(e) => setRestaurantForm({ ...restaurantForm, open: e.target.checked })} /><span>المطعم مفتوح</span></label>
            </div>
        ...
        <section className="panel selector">
          <div className="panel-head"><div><small>Restaurant</small><h2>{role === "admin" ? "اختر المطعم" : "مطعمك"}</h2></div><b>{restaurants.length}</b></div>
          {role === "admin" ? <div className="restaurant-tabs">{restaurants.map((item) => <button key={item.documentId} className={selectedId === item.documentId ? "active" : ""} onClick={() => setSelectedId(item.documentId)}>{getRestaurantName(item)}</button>)}</div> : null}
          {selectedRestaurant ? (
            <div className="restaurant-summary">
              ...
              <div className="restaurant-actions"><button className="danger" onClick={toggleRestaurant} disabled={saving}>{selectedRestaurant.open !== false && selectedRestaurant.active !== false ? "إطفاء المطعم وإخفاؤه" : "تشغيل المطعم وإظهاره"}</button>{role === "admin" ? <button onClick={() => editRestaurant(selectedRestaurant)}>تعديل التفاصيل</button> : null}</div>
            </div>
          ) : <div className="empty">ماكو مطعم مربوط بهذا الحساب.</div>}
        </section>

        <section className="layout">
          <section className="panel">
            <div className="panel-head"><div><small>Live Orders</small><h2>طلبات المطعم</h2></div><b>{orders.length}</b></div>
            {sortedOrders.length ? sortedOrders.slice(0, 40).map((order) => (
              <article className="card" key={order.documentId}>
                <div><h3>{order.customerName || order.customer || "زبون"}</h3><p>{order.address || "بدون عنوان"} — {order.phone || order.customerPhone || "بدون هاتف"}</p><small>#{order.orderId || order.documentId}</small></div>
            ...
            )) : <div className="empty"><h3>ماكو طلبات حالياً</h3><p>الطلب الجديد يظهر هنا مباشرة.</p></div>}
          </section>
